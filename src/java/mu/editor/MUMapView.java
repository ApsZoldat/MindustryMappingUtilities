...
public class MUMapView extends Element implements GestureListener{
    ...
    private Vec2[][] brushPolygons = new Vec2[MUMapEditor.brushSizes.length][0];
        ...
        addListener(new InputListener(){
            ...
            public boolean touchDown(InputEvent event, float x, float y, int pointer, KeyCode button){
                ...
                if(button == KeyCode.mouseRight){
                    lastTool = tool;
                    tool = MUEditorTool.eraser;
                }

                if(button == KeyCode.mouseMiddle){
                    lastTool = tool;
                    tool = MUEditorTool.zoom;
                }
            ...
            public void touchDragged(InputEvent event, float x, float y, int pointer){
                ...
                if(tool == MUEditorTool.line && tool.mode == 1){
    ...
    public Point2 project(float x, float y){
        ...
        if(editor.drawBlock.size % 2 == 0 && tool != MUEditorTool.eraser){
    ...
    public void draw(){
        ...
        int index = 0;
        for(int i = 0; i < MUMapEditor.brushSizes.length; i++){
            if(editor.brushSize == MUMapEditor.brushSizes[i]){
                index = i;
                break;
            }
        }
        ...
        if((!editor.drawBlock.isMultiblock() || tool == MUEditorTool.eraser) && tool != MUEditorTool.fill){
            if(tool == MUEditorTool.line && drawing){
            ...
            if((tool.edit || (tool == MUEditorTool.line && !drawing)) && (!mobile || drawing)){
                Point2 p = project(mousex, mousey);
                Vec2 v = unproject(p.x, p.y).add(x, y);

                //pencil square outline
                if(tool == MUEditorTool.pencil && tool.mode == 1){
            ...
            if((tool.edit || tool == MUEditorTool.line) && (!mobile || drawing)){
                Point2 p = project(mousex, mousey);
                Vec2 v = unproject(p.x, p.y).add(x, y);
    ...
    private boolean active(){
        ...
        && ui.editor.isShown() && tool == MUEditorTool.zoom &&
